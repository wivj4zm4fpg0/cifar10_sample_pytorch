...
def mkDataSet(data_size, data_length=50, freq=60., noise=0.00):
    ...
    for offset in range(data_size):
        train_x.append([[math.sin(2 * math.pi * (offset + i) / freq) + np.random.normal(loc=0.0, scale=noise)] for i in
                        range(data_length)])
...
for epoch in range(epochs_num):
    ...
    for i in range(int(test_size / batch_size)):
        offset = i * batch_size
        data = torch.tensor(test_x[offset:offset + batch_size])
        label = torch.tensor(test_t[offset:offset + batch_size])
    ...
    print(f'{epoch + 1} loss: {running_loss}, ={training_accuracy=}, {test_accuracy=}')
